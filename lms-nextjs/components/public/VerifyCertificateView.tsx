...
export default function VerifyCertificateView({ certificateId }: { certificateId: string }) {
      ...
      <div className="min-h-screen flex items-center justify-center bg-base-200 p-4">
        <div className="card bg-base-100 max-w-md w-full shadow-xl text-center p-8 border border-base-300">
          ...
          <p className="text-xs text-base-content/60 mt-2 mb-6">
            The certificate ID <strong>{certificateId}</strong> could not be verified in our official registry.
          </p>
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary/10 via-base-100 to-secondary/10 p-4">
      <div className="card bg-white text-slate-800 max-w-2xl w-full shadow-2xl border-4 border-amber-500/30 p-8 relative animate-fade-in">
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-slate-400 font-mono mt-1">REGISTRY VERIFICATION SUCCESSFUL</p>
        </div>

        <div className="text-center my-6 space-y-3">
          <p className="text-xs text-slate-500 uppercase tracking-wider">This is to officially verify that</p>
          ...
          <p className="text-xs text-slate-500 uppercase tracking-wider pt-2">has completed the certified course</p>
          <p className="text-lg font-bold text-amber-900">{course.title as string}</p>
        </div>

        <div className="flex justify-between items-end mt-8 pt-6 border-t border-slate-200 text-xs text-slate-500">
          <div>
            <p className="font-semibold text-slate-800">{issuer.name as string || "Course Director"}</p>
            <p className="text-[10px] text-slate-400">Authorized Signatory</p>
          </div>

          <div className="text-center">
            <p className="font-mono text-[11px] font-bold text-amber-800">{cert.certificateId as string}</p>
            <p className="text-[10px] text-slate-400">Public Credential ID</p>
          </div>

          <div className="text-right">
            <p className="font-semibold text-slate-800">
              {new Date(cert.issueDate as string || Date.now()).toLocaleDateString()}
            </p>
