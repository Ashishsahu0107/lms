...
export default function AssignmentDetails() {
  ...
  const currentSubmission = useSelector(
    (state) => state.assignments.currentSubmission,
  );
    ...
    return (
      ...
      >
        ...
        <p className="text-sm text-muted-foreground animate-pulse">
          Loading assignment briefing details...
        </p>
      </div>
    );
  }
  ...
  const formattedDueDate = new Date(currentAssignment.dueDate).toLocaleString(
    "en-US",
    {
      month: "short",
      day: "numeric",
  ...
  const handleSubmit = (e) => {
    ...
    if (!textAnswer.trim() && uploadedUrls.length === 0) {
      toast.error(
        "Please provide either a text response or upload at least one submission file.",
      );
      return;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start print:grid-cols-1 print:gap-4">
        ...
        <div className="lg:col-span-2 space-y-6 print:col-span-1 print:space-y-4">
          <Card className="border-base-300 shadow-xl rounded-3xl overflow-hidden bg-base-100 p-6 space-y-6 print:border-none print:shadow-none print:p-0">
            <div>
              <h2 className="text-lg font-black text-foreground border-b pb-3 mb-4">
                Assignment Briefing
              </h2>
              ...
              currentAssignment.rubric.length > 0 && (
                <div className="space-y-4 border-t pt-6 print:mt-2">
                  <h3 className="text-md font-bold text-foreground flex items-center gap-2">
                    <BookOpen className="h-4.5 w-4.5 text-primary" /> Evaluation
                    Rubric
                  </h3>
                  <div className="grid grid-cols-1 gap-3">
                    {currentAssignment.rubric.map((r, idx) => (
                      ...
                      >
                        ...
                        <span className="badge badge-primary rounded-xl font-bold py-2 px-3 shrink-0">
                          {r.maxPoints} pts
                        </span>
                      </div>
                    ))}
        ...
        <div className="space-y-6 print:col-span-1 print:space-y-4">
          ...
          <Card className="border-base-300 shadow-xl rounded-3xl bg-base-100 p-6 space-y-4 print:border-none print:shadow-none print:p-0">
            <h3 className="text-md font-black text-foreground border-b pb-2">
              Status Panel
            </h3>
            <div className="space-y-3 text-xs">
              <div className="flex justify-between items-center py-1">
                ...
                <span className="text-sm font-extrabold text-foreground">
                  {currentAssignment.totalMarks} Marks
                </span>
              </div>
              <div className="flex justify-between items-center py-1 border-t border-base-200">
                ...
                <span
                  className={`text-sm font-extrabold ${isClosed ? "text-error" : "text-foreground"}`}
                >
                  {formattedDueDate}
                </span>
              </div>
              <div className="flex justify-between items-center py-1 border-t border-base-200">
                ...
                {currentSubmission ? (
                  <span
                    className={`badge ${currentSubmission.status === "graded" ? "badge-success text-white" : currentSubmission.status === "late" ? "badge-warning" : "badge-info"} rounded-xl font-bold py-2.5 px-3 uppercase text-[10px]`}
                  >
                    {currentSubmission.status}
                  </span>
                ) : (
                  <span className="badge badge-secondary rounded-xl font-bold py-2.5 px-3 uppercase text-[10px]">
                    Unsubmitted
                  </span>
                )}
              </div>
          ...
          {currentSubmission && currentSubmission.status === "graded" && (
            <Card
              className="border-emerald-500/20 bg-emerald-500/5 shadow-xl rounded-3xl p-6 space-y-6 print:bg-transparent print:border-none print:shadow-none print:p-0"
              id="graded-report-card"
            >
              <div className="flex items-center gap-3">
              ...
              {currentSubmission.rubricEvaluation &&
                currentSubmission.rubricEvaluation.length > 0 && (
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2.5">
                      {currentSubmission.rubricEvaluation.map((re, idx) => (
                        <div
                          key={idx}
                          className="bg-base-100 border border-base-300/60 p-3 rounded-xl space-y-1 text-xs"
                        >
                          ...
                          {re.feedback && (
                            <p className="text-muted-foreground text-2xs italic leading-snug font-medium">
                              "{re.feedback}"
                            </p>
                          )}
                        </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative border-2 border-dashed border-base-300 rounded-2xl p-6 flex flex-col items-center justify-center gap-2 hover:border-primary/50 transition-colors bg-base-200/50 cursor-pointer">
                    ...
                    <span className="text-[10px] text-muted-foreground">
                      PDF, DOCX, PPTX, ZIP, Images up to 10MB
                    </span>
                  </div>
                ...
                {isClosed && (
                  <div className="flex items-center gap-1.5 justify-center text-[10px] text-error font-extrabold bg-error/10 py-2.5 rounded-xl">
                    <AlertTriangle className="h-3.5 w-3.5" /> Deadline passed!
                    Submissions closed.
                  </div>
                )}
              </form>
