  ...
  if (loading) {
    return (
      <div
        ...
        id="quiz-result-loading"
      >
        <div className="loading loading-spinner loading-lg text-primary"></div>
        <p className="text-sm text-muted-foreground animate-pulse">
          Analyzing scoring metrics and question review charts...
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="text-center py-20 bg-base-100 rounded-3xl border border-base-300 shadow-xl max-w-lg mx-auto mt-10">
        ...
        <p className="text-muted-foreground text-sm mb-6">
          The requested quiz attempt scorecard could not be found.
        </p>
        <Button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card className="border border-base-300 bg-base-100 shadow-2xl rounded-3xl overflow-hidden">
            <div className="p-8 bg-gradient-to-r from-primary/10 via-base-100 to-base-100 border-b border-base-300 flex items-center justify-between flex-wrap gap-4">
              ...
              <div className="text-right">
                <span
                  className={`inline-block text-lg font-black tracking-widest px-6 py-2.5 rounded-2xl border text-white capitalize ${
                    isPassed
            ...
            <CardContent className="p-8 space-y-8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="p-4 bg-base-200 rounded-2xl border border-base-300 text-center space-y-1">
                  ...
                  <span className="text-sm font-bold block capitalize text-foreground">
                    {attempt.status}
                  </span>
                </div>
              </div>
        ...
        {isPassed && (
          <div className="lg:col-span-1">
            <Card
              className="border-2 border-primary/20 bg-gradient-to-br from-primary/10 via-base-100 to-base-100 shadow-2xl rounded-3xl overflow-hidden"
              id="student-certificate-card"
            >
              <div className="p-6 text-center space-y-5">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Outstanding effort! You successfully passed the exam
                    guidelines and achieved system certification.
                  </p>
                </div>
                ...
                <div className="border border-base-300 bg-base-200 p-4 rounded-2xl text-[9px] font-mono leading-tight space-y-2 text-center text-muted-foreground relative overflow-hidden select-none">
                  ...
                  <div className="border border-primary/30 p-2.5 rounded-lg space-y-2">
                    <h5 className="font-extrabold text-primary tracking-widest text-[10px]">
                      LMS PRO CERTIFICATE
                    </h5>
                    <span className="block text-[7px]">
                      This is proudly presented to:
                    </span>
                    <span className="block font-bold text-foreground text-[9px] underline underline-offset-2">
                      {attempt.studentId?.name}
                    </span>
                    <span className="block text-[6px]">
                      For completing assessment parameters:
                    </span>
                    <span className="block font-bold text-foreground text-[8px] italic">
                      "{quiz.title}"
                    </span>
          ...
          {questions.map((question, index) => {
            ...
            if (!isSkipped) {
              if (
                ...
                if (
                  selected[0].toString().trim().toLowerCase() ===
            ...
            return (
              ...
              >
                <div className="p-5 bg-base-200/50 border-b border-base-300 flex justify-between items-center flex-wrap gap-2">
                  <span className="font-extrabold text-xs text-muted-foreground">
                    Question {index + 1} ({question.type.toUpperCase()})
                  </span>
                  <span
                    className={`badge text-white text-[10px] font-bold py-2.5 px-3.5 rounded-xl capitalize ${
                      isSkipped
                        ? "bg-warning"
                        : isCorrect
                          ? "bg-success"
                  ...
                  {(question.type === "short" ||
                    ...
                    <div className="space-y-3">
                      <div className="bg-base-200 border border-base-300 p-4 rounded-xl text-xs space-y-1">
                        <span className="font-bold text-muted-foreground block uppercase">
                          Your Response:
                        </span>
                        <p className="font-mono text-foreground leading-relaxed whitespace-pre-line bg-base-100 p-3 rounded-lg">
                          {selected[0] || (
                            <span className="text-warning/80 italic font-bold">
                              No response provided (Skipped)
                            </span>
                          )}
                        </p>
                      </div>
                      ...
                      <div className="bg-success/5 border border-success/20 p-4 rounded-xl text-xs space-y-1">
                        <span className="font-extrabold text-success block uppercase">
                          Expected Correct Solution Guidelines:
                        </span>
