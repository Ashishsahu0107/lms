...
import {
  ...
  RefreshCw, CheckCircle, XCircle, Clock, Plane, ArrowLeft, Info
} from "lucide-react";
...
export default function StudentAttendanceCalendar() {
  ...
  const totalClassesThisMonth = Object.values(calendarData).reduce((sum, list) => sum + list.length, 0);
  const presentRate = totalClassesThisMonth > 0
    ? Math.round(((monthlyCounts.present + monthlyCounts.late) / totalClassesThisMonth) * 100)
    : null;

  return (
    <div className="space-y-6 max-w-5xl mx-auto p-4 sm:p-6 lg:p-8" id="student-attendance-calendar-view">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-border pb-5">
        <div className="flex items-center gap-3">
          <Link to="/student/attendance">
            <Button variant="outline" size="sm" className="h-8 w-8 p-0 rounded-full border-border">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Monthly calendar view of your class presence</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={loadCalendar} disabled={loading} className="gap-2 text-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-card border border-border p-4 rounded-xl items-center shadow-sm">
        ...
        <div className="sm:col-span-2">
          <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1 block">Filter Course</label>
          <select
        ...
        <Card className="lg:col-span-2 border-border shadow-md overflow-hidden bg-card">
          <div className="grid grid-cols-7 border-b border-border bg-muted/20 text-center py-2">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((dayName) => (
              <span key={dayName} className="text-xs font-bold text-muted-foreground uppercase tracking-wider py-1">
                {dayName}
              </span>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 divide-y divide-x divide-border bg-base-100">
            {loading ? (
              Array.from({ length: 35 }).map((_, i) => (
                <div key={i} className="aspect-square p-2 animate-pulse bg-muted/10 min-h-[72px]" />
              ))
            ) : (
              daysGrid.map((day, idx) => {
                if (!day) return <div key={`empty-${idx}`} className="aspect-square bg-muted/5 min-h-[72px]" />;
                ...
                if (hasClasses) {
                  ...
                  if (statuses.includes("absent")) dayBg = "bg-rose-500/5 hover:bg-rose-500/10 border-rose-500/20";
                  else if (statuses.includes("late")) dayBg = "bg-amber-500/5 hover:bg-amber-500/10 border-amber-500/20";
                  else if (statuses.includes("leave")) dayBg = "bg-blue-500/5 hover:bg-blue-500/10 border-blue-500/20";
                  else dayBg = "bg-emerald-500/5 hover:bg-emerald-500/10 border-emerald-500/20";
                }

                return (
                  <button
                    key={`day-${day}`}
                    onClick={() => hasClasses && setSelectedDayDetails({ dateStr: dateKey, records })}
                    className={`aspect-square p-2 min-h-[72px] flex flex-col justify-between items-start transition-all ${dayBg} relative`}
                    disabled={!hasClasses}
                  >
                    <span className="text-xs font-bold text-foreground/60">{day}</span>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {records.map((r, i) => {
                        const dotColor = STATUS_CONFIG[r.status]?.color || "#94a3b8";
                        return (
                          <span
                            key={i}
        ...
        <div className="space-y-6">
          ...
          <Card className="border-border shadow-md bg-card">
            <CardHeader className="pb-3 border-b border-border bg-muted/10">
              <CardTitle className="text-sm font-bold text-foreground">Monthly Metrics</CardTitle>
            </CardHeader>
            <CardContent className="p-5 space-y-4">
              <div className="text-center pb-2 border-b border-border">
                <p className="text-3xl font-black text-indigo-500">{presentRate !== null ? `${presentRate}%` : "—"}</p>
                <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wider mt-1">Compliance Rate</p>
              </div>

              <div className="grid grid-cols-2 gap-3 text-center">
                {Object.entries(STATUS_CONFIG).map(([key, cfg]) => (
                  <div key={key} className="p-3 bg-muted/10 border border-border rounded-xl">
                    <p className="text-lg font-bold text-foreground">{monthlyCounts[key]}</p>
                    <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider mt-0.5">{cfg.label}</p>
                  </div>
                ))}
              </div>
            </CardContent>
                ...
                {selectedDayDetails ? (
                  ...
                  >
                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">
                      {new Date(selectedDayDetails.dateStr).toLocaleDateString("en-US", {
                        weekday: "short",
                        month: "short",
                        day: "numeric",
                      })}
                    ...
                    <div className="space-y-4">
                      {selectedDayDetails.records.map((r, i) => {
                        ...
                        return (
                          <div key={i} className="border border-border/80 bg-muted/5 p-3.5 rounded-xl space-y-2">
                            <h4 className="font-bold text-sm text-foreground leading-tight truncate">{r.courseTitle}</h4>
                            <div className="flex items-center gap-2">
                              <span className={`inline-flex items-center gap-1 text-[10px] font-extrabold uppercase tracking-wider px-2 py-0.5 rounded border ${cfg.cls}`}>
                  ...
                  <div className="text-center text-muted-foreground text-xs py-8">
                    Select a marked day in the calendar grid to view session notes and remarks.
                  </div>
                )}
              </AnimatePresence>
            </CardContent>
