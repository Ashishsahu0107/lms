  ...
  useEffect(() => {
    ...
    const handleNoteCreated = (data) => {
      if (data.courseId === selectedCourse) {
        ...
        toast.success(`New Notes Published: "${data.note.title}"!`, {
          duration: 4000,
        });
      }
    };
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/10 pb-5">
        <div>
          ...
          <p className="text-sm text-white/50 mt-1">
            Access rich summaries and download lecture resources uploaded by
            your teachers.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="py-12 flex items-center justify-center text-white/40 gap-2">
          <RefreshCw className="h-5 w-5 animate-spin" /> Loading classroom
          materials...
        </div>
        ...
        <div className="rounded-2xl border border-white/5 bg-white/5 p-12 text-center text-white/40 space-y-3">
          ...
          <p className="text-xs max-w-sm mx-auto">
            No study guides or summary outlines are uploaded for this course
            yet.
          </p>
        </div>
      ) : (
            ...
            >
              <div className="space-y-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-white/40 font-semibold">
                      Uploaded by: {note.teacherId?.name || "Instructor"}
                    </p>
                  </div>
                </div>
                ...
                <div className="text-xs text-white/70 leading-relaxed font-medium bg-black/20 p-4 rounded-xl border border-white/5 max-h-40 overflow-y-auto whitespace-pre-wrap">
                  {note.content ||
                    "No summary text provided. Download the notes document attachment."}
                </div>
              </div>
