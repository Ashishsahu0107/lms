  ...
  const [aiMessages, setAiMessages] = useState([
    {
      sender: "ai",
      content:
        "Hello! I am your AI study copilot. Ask me anything about the current video lecture!",
    },
  ]);
    ...
    try {
      ...
      if (res && res.success) {
        setAiMessages((prev) => [
          ...
          {
            sender: "ai",
            content: res.data?.reply || "I've processed your query.",
          },
        ]);
      } else {
        setAiMessages((prev) => [
          ...prev,
          {
            sender: "ai",
            content:
              "Apologies, I encountered a temporary logic glitch. Let me try again.",
          },
        ]);
      }
    } catch (err) {
      setAiMessages((prev) => [
        ...
        {
          sender: "ai",
          content: "Error connecting to AI helper. Please check your network.",
        },
      ]);
    } finally {
      setAiLoading(false);
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center text-white space-y-4">
        ...
        <p className="text-sm font-semibold tracking-wide animate-pulse">
          Launching Premium Netflix-style Course Player...
        </p>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 bg-[#070b13] flex flex-col z-50 text-white font-sans overflow-hidden">
      ...
      <div className="h-16 bg-slate-950/80 border-b border-white/5 backdrop-blur-md flex items-center justify-between px-6 shrink-0 z-20">
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm font-black text-white/90 truncate max-w-sm">
            {course?.title}
          </span>
          {currentTopic && (
            <>
              <span className="text-white/10">/</span>
              <span className="text-xs text-amber-400 font-bold bg-amber-500/10 px-2.5 py-1 rounded-full border border-amber-500/20 truncate max-w-xs">
                {currentTopic.title}
              </span>
            </>
          )}
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-black">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    className="text-white/80 hover:text-white h-10 w-10 bg-white/5 border border-white/5 rounded-full"
                    onClick={togglePlay}
                  >
                    {isPlaying ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button
                    ...
                    title="Bookmark active position"
                  >
          ...
          <div className="bg-slate-950 p-5 flex-shrink-0 border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4">
            <div>
              <h2 className="text-white font-extrabold text-base leading-tight truncate max-w-xl">
                {currentTopic?.title}
              </h2>
              <p className="text-xs text-slate-500 font-semibold mt-1">
                Topic {activeIndex + 1} of {allTopics.length} in this course
              </p>
            </div>
                    ...
                    {[
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[8px] mt-0.5 tracking-tight font-semibold uppercase">
                            {subOpt.label}
                          </span>
                        </button>
                      );
                      ...
                      <div className="flex flex-col h-full justify-between">
                        <div className="space-y-3 flex-1 overflow-y-auto max-h-[360px] pr-1 scrollbar-thin">
                          {aiMessages.map((msg, i) => (
                            ...
                            >
                              <span
                                className={`text-[10px] uppercase tracking-wider font-extrabold ${msg.sender === "user" ? "text-indigo-400" : "text-amber-400"}`}
                              >
                                {msg.sender === "user"
                                  ? "You"
                                  : "AI study assistant"}
                              ...
                              <div
                                className={`p-3 rounded-2xl text-xs font-semibold leading-relaxed max-w-[90%] border ${
                                  msg.sender === "user"
                          ...
                          {aiSummaryText ? (
                            ...
                            <div className="text-center py-10 space-y-3">
                              ...
                              <p className="text-slate-400">
                                Generate a comprehensive summary of this
                                lecture's material.
                              </p>
                            </div>
                          )}
                          ...
                          {!aiSummaryText ? (
                            ...
                            <>
                              <Button
                                ...
                                disabled={aiLoading}
                                variant="outline"
                                className="flex-1 border-slate-800 text-slate-300 font-semibold"
                              >
                                Regenerate
                              </Button>
                              <Button
                                onClick={() =>
                                  handleSaveAiNotesToMyNotes(
                                    aiSummaryText,
                                    "Lecture Summary",
                                  )
                                }
                        ...
                        <div className="flex-1 overflow-y-auto max-h-[360px] pr-1 space-y-3.5 scrollbar-thin text-xs text-slate-300 font-semibold leading-relaxed">
                          {aiExplainText ? (
                            ...
                            <div className="text-center py-10 space-y-3">
                              ...
                              <p className="text-slate-400">
                                Explain the core concepts of this topic in
                                simple terms with analogies.
                              </p>
                            </div>
                          )}
                          {aiLoading && (
                        ...
                        <div className="flex-1 overflow-y-auto max-h-[360px] pr-1 space-y-3.5 scrollbar-thin text-xs text-slate-300 font-semibold leading-relaxed">
                          {aiNotesText ? (
                            ...
                            <div className="text-center py-10 space-y-3">
                              <BookOpen className="h-8 w-8 text-slate-500 mx-auto" />
                              <p className="text-slate-400">
                                Generate structured study notes complete with
                                code templates.
                              </p>
                            </div>
                          )}
                        ...
                        <div className="flex-1 overflow-y-auto max-h-[360px] pr-1 space-y-4 scrollbar-thin">
                          {aiQuizQuestions.length > 0 ? (
                                    ...
                                    <div className="space-y-1.5">
                                      {q.options.map((opt, oIdx) => {
                                        const isSelected =
                                          selectedOption === oIdx;
                                        let btnClass =
                                          "w-full text-left p-2 rounded-lg border border-slate-800 bg-slate-950 text-slate-300 hover:bg-white/5 transition-all text-xs font-semibold";

                                        if (aiQuizSubmitted) {
                                          if (oIdx === q.correctOption) {
                                            btnClass =
                                              "w-full text-left p-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 text-xs font-bold";
                                          } else if (isSelected && !isCorrect) {
                                            btnClass =
                                              "w-full text-left p-2 rounded-lg border-rose-500/30 bg-rose-500/10 text-rose-400 text-xs font-bold";
                                          } else {
                                            btnClass =
                                              "w-full text-left p-2 rounded-lg border-slate-850 bg-slate-950 text-slate-600 text-xs font-semibold opacity-60";
                                          }
                                        } else if (isSelected) {
                                          btnClass =
                                            "w-full text-left p-2 rounded-lg border border-amber-500/50 bg-amber-500/10 text-amber-400 text-xs font-bold";
                                        }
                              ...
                              {aiQuizSubmitted && (
                                <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-center space-y-1">
                                  ...
                                  <p className="text-[10px] font-extrabold text-white">
                                    Score:{" "}
                                    {
                                      Object.keys(aiQuizAnswers).filter(
                                        (idx) =>
                                          aiQuizAnswers[idx] ===
                                          aiQuizQuestions[idx].correctOption,
                                      ).length
                                    }{" "}
                                    / {aiQuizQuestions.length} correct
                                  </p>
                                </div>
                              )}
                            </div>
                            ...
                            <div className="text-center py-10 space-y-3">
                              ...
                              <p className="text-slate-400">
                                Generate a live self-assessment quiz on this
                                topic.
                              </p>
                            </div>
                          )}
              ...
              {activeTab === "notes" && (
                <div className="space-y-4">
                  ...
                  <form
                    ...
                    className="space-y-3 p-3 border border-slate-800 rounded-xl bg-slate-900/40"
                  >
                    <textarea
                      value={noteContent}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        type="submit"
                        size="sm"
                        className="bg-indigo-600 hover:bg-indigo-700 text-xs font-semibold py-1 px-3"
                      >
                        Save Note
                      </Button>
                    </div>
                  ...
                  <div className="space-y-3">
                    <p className="text-[10px] text-slate-400 font-extrabold uppercase tracking-widest border-b border-white/5 pb-1">
                      Notes History
                    </p>
                    {notes.filter(
                      (n) =>
                        n.topicId.toString() === currentTopic?._id.toString(),
                    ).length === 0 ? (
                      <p className="text-xs text-slate-500 py-6 text-center">
                        No notes saved in this lesson.
                      </p>
                    ) : (
                      notes
                        .filter(
                          (n) =>
                            n.topicId.toString() ===
                            currentTopic?._id.toString(),
                        )
                        .map((note) => (
                          <div
                            key={note._id}
                            className="p-3 border border-slate-800 rounded-xl bg-slate-900/60 space-y-1"
                          >
                            <div className="flex justify-between items-center">
                              {note.videoPosition > 0 ? (
                                <button
                                  onClick={() =>
                                    handleSeekTo(note.videoPosition)
                                  }
                                  className="text-[10px] font-bold text-indigo-400 hover:underline flex items-center gap-1"
                                >
                                  <Clock className="h-3 w-3" />{" "}
                                  {formatTime(note.videoPosition)}
                                </button>
                              ) : (
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between border-b border-white/5 pb-2">
                    ...
                    <Button
                      ...
                      size="xs"
                      variant="outline"
                      className="border-indigo-500/30 text-indigo-400 hover:bg-indigo-600 hover:text-white font-semibold"
                    >
                      Add Bookmark
                    </Button>
                  </div>
                  {bookmarks.filter(
                    (b) =>
                      b.topicId.toString() === currentTopic?._id.toString(),
                  ).length === 0 ? (
                    <p className="text-xs text-slate-500 py-6 text-center">
                      No bookmarks added. Tap Bookmark button to highlight a
                      timestamp.
                    </p>
                  ) : (
                    <div className="space-y-2">
                      {bookmarks
                        ...
                        .map((b) => (
                          ...
                          >
                            <span className="text-xs font-semibold text-slate-200 truncate pr-3">
                              {b.title}
                            </span>
                            <span className="text-xs font-mono font-bold text-indigo-400 shrink-0">
                              {formatTime(b.videoPosition)}
                            </span>
                          </button>
                        ))}
                    </div>
                        ...
                        <div key={i} className="flex gap-2.5 items-start">
                          ...
                          <div className="min-w-0 flex-1 space-y-0.5">
                            <div className="flex justify-between items-baseline">
                              <span className="text-xs font-bold text-slate-300 truncate pr-2">
                                {msg.user?.name}
                              </span>
                            ...
                            <p className="text-xs text-slate-400 font-semibold leading-relaxed break-words">
                              {msg.content}
                            </p>
                          </div>
                        </div>
              ...
              {activeTab === "resources" && (
                <div className="space-y-3">
                  ...
                  currentTopic.resources.length === 0 ? (
                    <p className="text-xs text-slate-500 py-6 text-center">
                      No downloadable resources configured for this topic.
                    </p>
                  ) : (
                    <div className="space-y-2">
              ...
              <div className="max-h-[160px] overflow-y-auto space-y-2 pr-1 scrollbar-thin">
                {course?.modules?.map((mod) => (
                  <div key={mod._id} className="space-y-1">
                    <p className="text-[10px] font-extrabold text-slate-500 truncate uppercase tracking-widest">
                      {mod.title}
                    </p>
                    <div className="space-y-1">
                      {mod.topics?.map((topic) => {
                        const isCompleted = progress?.lectureProgress?.some(
                          (l) =>
                            l.lectureId.toString() === topic._id.toString() &&
                            l.completed,
                        );
