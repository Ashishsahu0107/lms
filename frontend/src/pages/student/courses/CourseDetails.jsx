...
const dummyCourse = {
  ...
  instructor: {
    ...
    avatar: "",
    bio: "Senior Software Engineer with 15 years of experience",
  },
  ...
  thumbnail:
    "https://images.unsplash.com/photo-1627392662291-4c2ac9c424e9?w=800",
  description:
    "Master advanced JavaScript concepts including closures, prototypes, async programming, and design patterns.",
  about:
    "This comprehensive course takes you from intermediate to advanced JavaScript developer. You'll learn powerful patterns and techniques used in production applications.",
  progress: 45,
  enrolled: true,
...
export default function CourseDetails() {
  ...
  const totalDuration = course.sections.reduce(
    (acc, sec) => acc + sec.lectures.reduce((a, l) => a + l.duration, 0),
    0,
  );
          ...
          <div className="flex-1 text-white space-y-4">
            ...
            <div className="flex flex-wrap gap-4 text-sm text-white/80">
              <span className="flex items-center gap-1">
                <Star className="h-4 w-4 text-amber-400" /> {course.rating}{" "}
                rating
              </span>
              <span className="flex items-center gap-1">
      ...
      >
        {[
          { icon: BookOpen, label: "Total Lectures", value: course.lectures },
          {
            icon: CheckCircle2,
            label: "Completed",
            value: course.completedLectures,
          },
          {
            icon: Clock,
          ...
          },
          {
            icon: Award,
            label: "Certificate",
            value: course.enrolled ? "Available" : "After Completion",
          },
        ].map((stat, i) => (
          <Card key={i}>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {course.sections.map((section, sIdx) => {
                  const completedInSection = section.lectures.filter(
                    (l) => l.completed,
                  ).length;
                  return (
                    <div
                      ...
                      >
                        <div className="text-left">
                          ...
                          <p className="text-sm text-muted-foreground">
                            ...
                            {completedInSection}/{section.lectures.length}{" "}
                            completed
                          </p>
                        </div>
                      ...
                      {expandedSections[sIdx] && (
                        <div className="divide-y">
                          {section.lectures.map((lecture) => (
                            ...
                            >
                              <div
                                className={`w-8 h-8 rounded-full flex items-center justify-center ${lecture.completed ? "bg-emerald-100" : "bg-muted"}`}
                              >
                                {lecture.completed ? (
