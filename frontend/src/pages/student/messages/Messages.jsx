  ...
  const handleSelectConversation = async (conv) => {
    ...
    try {
      ...
      setConversations((prev) =>
        prev.map((c) =>
          c.participant?._id === conv.participant._id
            ? { ...c, unreadCount: 0 }
            : c,
        ),
      );
        ...
        if (selectedConv.type === "group" && msg.groupId === selectedConv.id) {
          ...
          const recipientId =
            typeof msg.recipientId === "object"
              ...
              : msg.recipientId;
          const partnerId =
            senderId === currentUser.id ? recipientId : senderId;
  ...
  const handleOpenNewChatModal = async () => {
    ...
    try {
      ...
      if (currentUser.role === "student") {
        ...
        list = Array.from(teacherMap.values()).map((t) => ({
          ...t,
          role: "Instructor",
        }));
      } else {
  ...
  const handleStartChatWithContact = (contact) => {
    ...
    if (existing) {
      ...
      const mockConv = {
        participant: contact,
        lastMessage: {
          content: "Start of your new chat history.",
          createdAt: new Date(),
        },
        unreadCount: 0,
      };
  ...
  const getGroupTypingText = () => {
    ...
    Object.keys(typingUsers).forEach((key) => {
      ...
      if (gId === selectedConv.id) {
        const m = selectedConv.members?.find(
          (member) => (member._id || member) === uId,
        );
        ...
        <Card className="h-full flex flex-col overflow-hidden border-base-300 shadow-xl bg-base-100">
          <CardHeader className="p-4 border-b border-base-300 flex flex-col gap-3">
            ...
            <div className="relative">
              ...
              <Input
                className="pl-9 h-9 border-base-300 bg-base-200/50"
                placeholder="Search..."
                value={searchQuery}
            ...
            {activeSidebarTab === "direct" ? (
              loadingConvs ? (
                ...
                filteredConvs.map((conv) => {
                  ...
                  const isUserOnline =
                    onlineUsers.has(partner._id) || partner.isOnline;
                  const active =
                    selectedConv?.type === "direct" &&
                    selectedConv.id === partner._id;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p
                          className={`text-xs truncate mt-1 ${conv.unreadCount > 0 ? "font-bold text-base-content" : "text-base-content/60"}`}
                        >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs truncate text-base-content/60 mt-1">
                        {group.description || "Group chat thread"}
                      </p>
                    </div>
                  </button>
                );
            ...
            <CardHeader className="p-4 border-b border-base-300 flex-row items-center gap-3 shrink-0 bg-base-200/20">
              <div className="relative">
                <Avatar className="w-10 h-10 border border-base-300">
                  <AvatarImage
                    src={
                      selectedConv.avatar || selectedConv.participant?.avatar
                    }
                  />
                  <AvatarFallback className="bg-primary/20 text-primary">
                    {selectedConv.type === "group"
                ...
                {selectedConv.type === "direct" &&
                  (onlineUsers.has(selectedConv.id) ||
              ...
              <Button variant="ghost" size="icon" className="rounded-full">
                <MoreVertical className="h-4 w-4 text-base-content" />
              </Button>
            </CardHeader>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-base-200/30 scrollbar-thin">
              {loadingMessages ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-2">
                  ...
                  <p className="text-xs text-base-content/60">
                    Send a greeting message to open the dialog.
                  </p>
                </div>
              ) : (
                    ...
                    >
                      ...
                      <div className={`max-w-[70%] ${isMe ? "order-1" : ""}`}>
                        {selectedConv.type === "group" && !isMe && (
                          <span className="text-[10px] text-base-content/50 block mb-1 font-semibold ml-1">
                            {senderName}
                          </span>
                        )}
                        <div
                          className={`rounded-2xl px-4 py-2.5 shadow-sm text-sm ${isMe ? "bg-primary text-primary-content rounded-tr-none" : "bg-base-100 border border-base-300 rounded-tl-none text-base-content"}`}
                        >
                          {msg.content && (
                        ...
                        <p className="text-[9px] text-base-content/50 mt-1 flex items-center justify-end gap-1">
                          ...
                          {isMe && (
                            <CheckCheck
                              className={`h-3.5 w-3.5 ${msg.read ? "text-primary" : "text-base-content/30"}`}
                            />
                          )}
                        </p>
                      </div>
              ...
              {pendingAttachment && (
                <div className="flex items-center justify-between bg-primary/5 p-2 px-3 rounded-xl border border-primary/20 text-xs">
                  <div className="flex items-center gap-2 truncate">
                    ...
                    <span className="truncate text-base-content font-bold">
              ...
              <div className="flex items-center gap-2">
                ...
                <label className="btn btn-ghost btn-circle btn-sm hover:bg-primary/10 text-base-content/60 hover:text-primary cursor-pointer flex items-center justify-center shrink-0">
                  ...
                  <input
                    ...
                    accept=".pdf,.docx,.doc,.zip,.rar,.txt,.pptx"
                  />
                </label>
                ...
                {isRecording ? (
                  <div className="flex-1 flex items-center gap-3 bg-red-500/10 border border-red-500/20 px-3 py-2 rounded-xl text-red-500 text-xs font-semibold animate-pulse">
                    ...
                    <div className="flex gap-0.5 items-center">
                      ...
                      />
                      <span
                        className="w-1 h-2 bg-red-500 rounded-full animate-bounce"
                        style={{ animationDelay: "300ms" }}
                      />
                    </div>
                    <span>
                    ...
                    <button
                      ...
                      className="ml-auto bg-red-500 text-white px-2.5 py-1 rounded-lg hover:bg-red-600 transition-all text-[10px] cursor-pointer"
                    >
                      Stop & Send
                    </button>
                  </div>
                ...
                <Button
                  ...
                  onClick={sendMessage}
                  disabled={
                    uploadingFile || (!messageText.trim() && !pendingAttachment)
                  }
                >
              ...
              {activeModalTab === "direct" ? (
                ...
                <div className="flex-1 overflow-y-auto space-y-4 mt-2 pr-1 scrollbar-thin">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      className="border-base-300"
                      placeholder="Describe the group scope..."
                      value={groupDesc}
                    ...
                    <div className="space-y-1 max-h-48 overflow-y-auto border border-base-300 p-2 rounded-xl bg-base-200/30 scrollbar-thin">
                      {loadingContacts ? (
                        <div className="text-center py-6 text-xs text-base-content/60">
                          ...
                          (c) => c._id !== "support-agent-id-placeholder",
                        ).length === 0 ? (
                        <div className="text-center py-6 text-xs text-base-content/60">
                        ...
                        availableContacts
                          ...
                          .map((contact) => (
                            <label
                              key={contact._id}
                              className="flex items-center gap-3 p-2 hover:bg-base-200/50 rounded-lg cursor-pointer text-sm"
                            >
                              <input
                                type="checkbox"
                                ...
                                onChange={() =>
                                  handleToggleGroupMember(contact._id)
                                }
                              />
                              <Avatar className="w-8 h-8 border border-base-300 shrink-0">
                                <AvatarFallback className="bg-secondary/20 text-secondary">
                  ...
                  <Button
                    className="w-full mt-4 btn-primary text-primary-content"
