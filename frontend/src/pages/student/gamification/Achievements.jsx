...
export default function Achievements() {
  ...
  useEffect(() => {
    ...
    const handleAchievementUnlocked = (data) => {
      // Show real-time beautiful custom toast!
      toast(
        () => (
          <div className="flex flex-col gap-1 text-xs">
            ...
            <span className="font-semibold text-white/90">
              Congratulations! You unlocked the **{data.latestAchievement.title}
              ** badge!
            </span>
          </div>
        ),
  ...
  const handleClaimReward = (badge) => {
    toast.success(
      `Claimed reward coupon for unlocking '${badge}' badge! Check your email!`,
    );
  };
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/10 pb-5">
        <div>
          ...
          <p className="text-sm text-white/50 mt-1">
            Unlock badges, maintain daily streaks, accumulate XP, and unlock
            learning milestones.
          </p>
        </div>
      ...
      {loading ? (
        <div className="py-12 flex items-center justify-center text-white/40 gap-2">
          <RefreshCw className="h-5 w-5 animate-spin" /> Loading achievements
          board...
        </div>
      ) : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            ...
            >
              <div className="space-y-1">
                ...
                <p className="text-xs text-white/50 font-medium">
                  Keep completing assignments to fuel the streak fire!
                </p>
              </div>
            ...
            >
              <div className="space-y-1">
                ...
                <p className="text-4xl font-black text-white mt-1">
                  {(userData?.xp ?? 15).toLocaleString()}
                </p>
                <p className="text-xs text-white/50 font-medium">
                  Accumulating points unlocks rare milestone badges!
                </p>
              </div>
            ...
            >
              <div className="space-y-1">
                ...
                <p className="text-xs text-white/50 font-medium">
                  Unlocks learning coupon discounts on new courses!
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 rounded-2xl border border-white/10 bg-black/40 p-6 backdrop-blur-xl space-y-4">
              <h2 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
                <Award className="h-5 w-5 text-purple-400" /> Milestone Badges
                Catalog
              </h2>

              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {userData?.badges?.length === 0 ? (
                  <p className="text-white/40 text-xs col-span-full py-8 text-center">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <span className="text-[8px] uppercase tracking-widest font-extrabold text-white/40 block mt-1">
                            Unlocked
                          </span>
                        </div>
                        <button
            ...
            <div className="rounded-2xl border border-white/10 bg-black/40 p-6 backdrop-blur-xl flex flex-col h-[400px]">
              <h2 className="text-sm font-bold text-white uppercase tracking-wider mb-4 flex items-center gap-1.5">
                <Sparkles className="h-5 w-5 text-amber-400" /> Unlock Log
                History
              </h2>
              ...
              <div className="flex-1 overflow-y-auto space-y-4 pr-1 scrollbar-thin">
                {userData?.achievements?.length === 0 ? (
                  <div className="text-white/30 text-xs py-8 text-center">
                    Complete quizzes to unlock achievements milestones!
                  </div>
                ) : (
