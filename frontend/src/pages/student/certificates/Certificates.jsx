...
export default function Certificates() {
  ...
  return (
    <motion.div
      ...
      id="student-certificates-dashboard"
    >
      {/* Header */}
      <motion.div
        variants={item}
      ...
      >
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Your verified course achievements and credentials
          </p>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="animate-pulse bg-muted/20 border border-border rounded-2xl h-72"
              />
            ))}
          </div>
        ) : certs.length === 0 ? (
          <div className="text-center py-20 bg-muted/10 border border-border border-dashed rounded-3xl space-y-3">
            ...
            <p className="text-sm text-muted-foreground max-w-sm mx-auto">
              Complete your course materials up to 90% and ask your instructor
              to issue your official certificate.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {certs.map((cert) => (
              <Card
                key={cert._id}
                className="overflow-hidden hover:shadow-xl transition-all duration-300 border-border group relative"
              >
                <div className="relative h-40 bg-gradient-to-br from-indigo-900 to-purple-900 flex items-center justify-center p-6 text-center text-white">
                  ...
                  <div className="relative z-10 space-y-2">
                    ...
                    <h4 className="font-bold text-xs uppercase tracking-widest text-amber-200">
                      Completion Certificate
                    </h4>
                    <p className="font-semibold text-xs line-clamp-2 max-w-xs">
                      {cert.course?.title}
                    </p>
                  </div>
                ...
                <CardContent className="p-4 space-y-3 bg-card">
                  ...
                  <div className="flex gap-2 pt-2">
                    <Button
                      size="sm"
                      className="flex-1 gap-1 text-xs bg-indigo-600 text-white hover:bg-indigo-700 font-semibold"
      ...
      >
        {selectedCert && (
          <div
            className="rounded-xl overflow-hidden border border-border bg-white text-slate-800 shadow-2xl relative"
            ...
            <div className="p-12 text-center space-y-8 relative border-[12px] border-amber-500/20 m-4 rounded bg-amber-50/10">
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs text-slate-400 max-w-md mx-auto">
                  A comprehensive professional course validation, confirming
                  full mastery of core subjects and outstanding overall syllabus
                  participation.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-end pt-12 gap-6 border-t border-slate-200">
                <div className="text-left space-y-1">
                  ...
                  <p className="font-bold text-sm text-slate-700 font-serif">
                    {selectedCert.issuedBy?.name || "LMS Platform Team"}
                  </p>
                  <p className="text-[10px] text-indigo-600 font-bold uppercase tracking-wider">
                    {selectedCert.issuedBy?.role?.replace("_", " ")}
                  </p>
                </div>
                <div className="text-center space-y-1 bg-amber-500/5 px-4 py-2 rounded-lg border border-amber-500/10 flex flex-col items-center justify-center gap-1.5 shrink-0">
                  ...
                  <p className="font-mono text-[9px] text-slate-800 font-bold">
                ...
                <div className="text-right space-y-1">
                  ...
                  <p className="text-[10px] text-emerald-600 font-bold uppercase">
                    Progress: {selectedCert.completionPercentage}%
                  </p>
                </div>
              </div>
            </div>
