    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            <CheckSquare className="h-5 w-5 text-emerald-400" /> Attendance &
            Student Engagement Analytics
          </h2>
          <p className="text-xs text-white/50">
            Monitor student presence statistics, course-wise compliance rates,
            and trigger low attendance warning flags.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-white/10 bg-black/40 p-6 backdrop-blur-xl">
          <h3 className="text-sm font-semibold tracking-wider text-white/50 uppercase mb-4 flex items-center gap-1.5">
            <CheckSquare className="h-4 w-4 text-emerald-400" /> Daily
            Compliance & Presence Curve
          </h3>
          <div className="h-80 w-full">
            {loading ? (
              <div className="h-full flex items-center justify-center text-white/30">
                Loading presence trends...
              </div>
            ) : (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data?.courseAttendance} layout="vertical">
                  ...
                  <YAxis
                    ...
                    fontSize={8}
                    width={80}
                    tickFormatter={(tick) => tick.substring(0, 12) + ".."}
                  />
                  <Tooltip
                    contentStyle={{
                      backgroundColor: "#171717",
      ...
      <div className="rounded-2xl border border-rose-500/20 bg-rose-500/5 p-6 backdrop-blur-xl">
        <h3 className="text-sm font-bold tracking-wider text-rose-400 uppercase flex items-center gap-2 mb-4">
          <AlertCircle className="h-5 w-5" /> Low Attendance Student Alerts
          (Compliance Risk)
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading ? (
            <div className="text-white/30 text-xs col-span-full">
              Loading warning alerts...
            </div>
          ) : data?.alerts?.length === 0 ? (
            <div className="text-emerald-400/60 text-xs font-semibold col-span-full">
              Excellent! All enrolled students satisfy attendance requirements.
            </div>
          ) : (
            data?.alerts?.map((alert, idx) => (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <p className="text-[8px] uppercase tracking-wider text-rose-400/50 font-bold">
                    compliance
                  </p>
                </div>
              </motion.div>
