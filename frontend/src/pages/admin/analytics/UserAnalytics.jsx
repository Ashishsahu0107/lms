      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            <Users className="h-5 w-5 text-blue-400" /> User Analytics &
            Audience Demographics
          </h2>
          <p className="text-xs text-white/50">
            Track student registrations, active session trends, and platform
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-white/10 bg-black/40 p-6 backdrop-blur-xl">
          <h3 className="text-sm font-semibold tracking-wider text-white/50 uppercase mb-4 flex items-center gap-1.5">
            <BarChart3 className="h-4 w-4" /> Platform Usage Heatmap (Active
            Hour Density)
          </h3>
          <div className="grid grid-cols-8 gap-1.5 text-center text-xs font-semibold select-none">
            {/* Hour headers */}
            <div className="h-8 flex items-center justify-center text-white/30">
              Day
            </div>
            ...
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => {
              const dayValues =
                data?.heatmap?.filter((h) => h.day === day) || [];
              return (
