...
import { getTeacherStats, getTeacherAnalytics } from "../../../services/dashboardService";
import {
  Users,
  BookOpen,
  TrendingUp,
...
// Beautiful static fallback for revenue tracking over past months
...
export default function TeacherDashboard() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center py-32 space-y-4" id="teacher-dashboard-loading">
        <Loader2 className="h-12 w-12 animate-spin text-teal-600" />
        <p className="text-muted-foreground text-sm font-medium animate-pulse">Syncing course analytics and submissions...</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative z-10">
          <Badge variant="secondary" className="mb-4 bg-white/20 text-white border-0 hover:bg-white/30 backdrop-blur-md">
            <Sparkles className="h-3 w-3 mr-1.5 fill-current" /> Instructor Command Center
          </Badge>
          <h1 className="text-3xl font-bold text-white mb-2" id="teacher-welcome-title">
            Welcome back, {user?.name || "Educator"}!
          </h1>
          <p className="text-white/80 max-w-xl">
            You are teaching {stats.totalCourses} active courses. Your class average progress rate is {analytics.averageProgress}% completed. Keep setting high marks!
          </p>
        </div>
      ...
      {/* Quick Stats Grid */}
      <motion.div variants={item} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" id="teacher-stats-grid">
        <StatCard
          ...
          changeType="positive"
          icon={BookOpen}
          trend={stats.totalCourses > 0 ? Math.min(stats.totalCourses * 15, 100) : 0}
        ...
        <div className="rounded-xl border bg-card p-6 shadow-card hover:shadow-elevated transition-all duration-300 relative overflow-hidden bg-gradient-to-br from-amber-500/10 via-card to-amber-600/5 border-amber-500/20">
          <div className="flex items-start justify-between">
            <div className="space-y-2">
              <p className="text-sm font-medium text-amber-600 dark:text-amber-400">Review Queue</p>
      ...
      {/* Quick Action Drawer */}
      <motion.div variants={item} className="grid grid-cols-1 md:grid-cols-3 gap-4" id="teacher-quick-actions">
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={analytics.courseEnrollment} layout="vertical">
                    ...
                    <XAxis type="number" className="text-xs text-muted-foreground font-medium" />
                    <YAxis dataKey="course" type="category" className="text-xs text-muted-foreground font-medium" width={90} />
                    <Tooltip
                      contentStyle={{
                    ...
                    />
                    <Bar dataKey="students" fill="#0d9488" radius={[0, 6, 6, 0]} barSize={16} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
          ...
          <CardContent>
            {stats.recentSubmissions.length === 0 ? (
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="font-semibold text-foreground">No recent submissions found</p>
                <p className="text-xs mt-1">Students have not posted assignments for your classes recently.</p>
              </div>
            ) : (
                  ...
                  <tbody>
                    {stats.recentSubmissions.map((sub) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          <span className="font-medium text-muted-foreground line-clamp-1">{sub.assignmentTitle}</span>
                        </td>
                        ...
                        <td className="py-3 px-4 text-right">
                          <Link to="/teacher/assignments">
                            <Button size="xs" variant="outline" className="text-xs h-7 px-2.5">
                              Review
                            </Button>
                          </Link>
                        </td>
