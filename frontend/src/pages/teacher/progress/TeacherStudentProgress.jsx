...
export default function TeacherStudentProgress() {
  ...
  useEffect(() => {
    ...
    const handleProgressUpdated = (data) => {
      ...
      toast.success(
        `Live sync: ${data.studentName || "A student"} completed a lesson!`,
        {
          icon: "⚡",
          style: {
    ...
    const handleQuizSubmitted = (data) => {
      toast.success(
        `Live sync: Quiz completed by ${data.studentName || "learner"}!`,
        {
          icon: "🎯",
        },
      );
    ...
    >
      ...
      >
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Monitor real-time syllabus learning rates, completed topics counts,
            and active daily presence.
          </p>
        </div>
      ...
      >
        ...
        <Card className="lg:col-span-2 border-border hover:shadow-md transition-all">
          <CardHeader>
            <CardTitle className="text-base text-foreground flex items-center gap-2">
              <TrendingUp className="h-5 w-5 text-blue-500" /> Syllabus Progress
              Trend
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-border hover:shadow-md transition-all bg-card">
          <CardHeader>
            <CardTitle className="text-base text-foreground flex items-center gap-2">
              <Activity className="h-5 w-5 text-emerald-500" /> Learning Growth
              & Engagement
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <motion.div
        variants={item}
        className="flex justify-between items-center gap-4 bg-card border border-border p-4 rounded-xl shadow-sm"
      >
      ...
      <motion.div variants={item} className="grid grid-cols-1 gap-4">
        {filteredStudents.map((student) => (
          <Card
            key={student.id}
            className="border-border shadow-sm hover:shadow-md transition-all bg-card"
          >
            <CardContent className="p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <div className="shrink-0 flex items-center gap-2">
                <Badge
                  variant="secondary"
                  className="font-semibold text-xs border-0 px-3 py-0.5"
                >
              ...
              <div className="text-right shrink-0">
                <span className="text-xs text-muted-foreground flex items-center justify-end gap-1 select-none font-medium">
                  <Clock className="h-3.5 w-3.5 text-muted-foreground" /> Active{" "}
                  {student.lastActive}
                </span>
              </div>
            </CardContent>
