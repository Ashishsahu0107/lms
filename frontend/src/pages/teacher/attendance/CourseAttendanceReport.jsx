...
export default function CourseAttendanceReport() {
  ...
  const loadReport = useCallback(async (courseId) => {
    ...
    try {
      ...
      const studentsRes = await getCourseAttendanceStudents(
        courseId,
  ...
  useEffect(() => {
    if (selectedCourse) loadReport(selectedCourse);
  }, [selectedCourse, loadReport]);

  const filtered = studentReports.filter(
    ...
    <div className="space-y-6 max-w-7xl mx-auto" id="course-report-page">
      ...
      <div className="border-b border-border pb-5">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Per-student attendance summary for your courses
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            },
            {
              ...
              value: studentReports.filter((s) => s.status === "danger").length,
              icon: AlertTriangle,
              color: "#f43f5e",
            },
            {
              label: "At Risk",
              value: studentReports.filter((s) => s.status === "warning")
                .length,
              icon: TrendingDown,
              color: "#f59e0b",
            },
            {
              label: "Safe",
              value: studentReports.filter((s) => s.status === "safe").length,
              icon: CheckCircle,
              color: "#10b981",
            },
          ].map((stat) => (
      ...
      <Card className="border-border shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-border bg-muted/20 flex items-center gap-2">
          ...
          {studentReports.length > 0 && (
            <Badge className="ml-auto bg-muted/40 text-muted-foreground text-xs">
              {studentReports.length} students
            </Badge>
          )}
        </div>
              ...
              {loading ? (
                ...
                filtered.map((student, idx) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${statusBadge(student.status)}`}
                      >
