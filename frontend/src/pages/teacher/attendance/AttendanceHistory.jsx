...
import {
  Calendar, ChevronLeft, ChevronRight, Filter, ClipboardList,
  History, BarChart3, CheckCircle, XCircle, Clock, Plane, RefreshCw
} from "lucide-react";
...
function RateBar({ rate }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-bold min-w-[36px] text-right" style={{ color }}>{rate}%</span>
    </div>
  );
}
...
export default function AttendanceHistory() {
  ...
  useEffect(() => {
    if (selectedCourse) loadHistory(selectedCourse, { from, to, page, limit: 20 });
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto" id="attendance-history-page">
      ...
      <div className="border-b border-border pb-5">
        ...
        <p className="text-sm text-muted-foreground mt-1">View past attendance sessions by course</p>
      </div>
      ...
      <Card className="border-border shadow-sm">
        <CardContent className="p-4">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
            <div>
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5 block">Course</label>
              {coursesLoading ? (
            ...
            <Button onClick={handleFilter} disabled={loading} className="bg-emerald-600 hover:bg-emerald-700 text-white h-10">
      ...
      {pagination.total > 0 && (
        <div className="flex items-center gap-3 text-sm text-muted-foreground">
          ...
          <span><strong className="text-foreground">{pagination.total}</strong> session(s) found</span>
        </div>
      )}
      ...
      <Card className="border-border shadow-sm overflow-hidden">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                {["Date", "Total", "Present", "Absent", "Late", "Leave", "Rate"].map((h) => (
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                sessions.map((session, idx) => (
                  <motion.tr key={session.date} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    transition={{ delay: idx * 0.03 }} className="hover:bg-primary/5 transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <Calendar className="h-4 w-4 text-emerald-500 flex-shrink-0" />
                        <span className="font-semibold text-sm text-foreground">{formatDate(session.date)}</span>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-sm text-foreground font-medium">{session.total}</td>
                    <td className="px-5 py-4">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${STATUS_CONFIG.present.cls}`}>
                        {session.present}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${STATUS_CONFIG.absent.cls}`}>
                        {session.absent}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${STATUS_CONFIG.late.cls}`}>
                        {session.late}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${STATUS_CONFIG.leave.cls}`}>
                        {session.leave}
                      </span>
                    </td>
        ...
        {pagination.pages > 1 && (
          <div className="px-5 py-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
            <span>Page {page} of {pagination.pages} ({pagination.total} sessions)</span>
            <div className="flex gap-2">
              <Button size="sm" variant="outline" onClick={() => setPage((p) => Math.max(1, p - 1))}
                disabled={page === 1} className="h-7 w-7 p-0">
                <ChevronLeft className="h-3.5 w-3.5" />
              </Button>
              <Button size="sm" variant="outline" onClick={() => setPage((p) => Math.min(pagination.pages, p + 1))}
                disabled={page === pagination.pages} className="h-7 w-7 p-0">
