...
import { Sparkles, Brain, Cpu, BookOpen, CheckCircle, RefreshCw, Zap, GraduationCap } from "lucide-react";
    ...
    if (currentQuestion + 1 < quizQuestions.length) {
      ...
      toast(() => (
        <span className="flex items-center gap-1.5 text-xs font-bold text-amber-500">
          <Zap className="h-4 w-4 text-amber-400 animate-pulse" /> Quiz complete! Awarded +10 XP streak bonus!
        </span>
      ), { duration: 4000 });
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/10 pb-5">
        <div>
          <h1 className="text-3xl font-extrabold text-white bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400 bg-clip-text text-transparent flex items-center gap-2">
            <Brain className="h-8 w-8 text-blue-400 animate-pulse" /> AI recommendations & Quiz Labs
          </h1>
          <p className="text-sm text-white/50 mt-1">Get AI-customized study pathways and build topic multiple-choice testing drills instantly.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-white/10 bg-black/40 p-6 backdrop-blur-xl space-y-6 flex flex-col justify-between">
          <div className="space-y-4">
            <h2 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
              <Cpu className="h-5 w-5 text-blue-400" /> AI Classroom Quiz Generator
            </h2>
            ...
            <form onSubmit={handleGenerateQuiz} className="flex gap-2">
              ...
              >
                {loadingQuiz ? <RefreshCw className="h-4 w-4 animate-spin" /> : "Build Questions"}
              </button>
            </form>
            ...
            <div className="rounded-2xl border border-white/5 bg-black/20 p-5 min-h-[250px] flex flex-col justify-between">
              {loadingQuiz ? (
                <div className="flex-1 flex flex-col items-center justify-center text-white/40 text-xs gap-2">
                  <RefreshCw className="h-6 w-6 animate-spin text-blue-400" /> Generating 3 MCQs on '{topic}'...
                </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-white/30 text-xs text-center space-y-2">
                  ...
                  <p className="font-semibold text-white/50">Questions Laboratory Ready</p>
                  <p className="text-[10px]">Type any syllabus subject above to generate custom MCQs study drills.</p>
                </div>
              ) : quizCompleted ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
                  <Sparkles className="h-12 w-12 text-amber-400 animate-bounce" />
                  <div>
                    <h3 className="font-black text-white text-lg">Congratulations!</h3>
                    <p className="text-xs text-white/50 mt-1">
                      You scored <span className="text-amber-400 font-bold">{score}/{quizQuestions.length}</span> correct answers on {topic}!
                    </p>
                  </div>
                  <button
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {quizQuestions[currentQuestion].options.map((opt, oIdx) => {
                        ...
                        const isCorrect = oIdx === quizQuestions[currentQuestion].correctOption;
                        let optStyle = "border-white/10 bg-white/5 text-white hover:bg-white/10";
                        
                        if (selectedOption !== null) {
                          if (isCorrect) optStyle = "border-emerald-500/30 bg-emerald-500/20 text-emerald-400 font-bold";
                          else if (isSelected) optStyle = "border-rose-500/30 bg-rose-500/20 text-rose-400";
                          else optStyle = "border-white/5 bg-white/5 opacity-55 text-white/70";
                        }
                  ...
                  {selectedOption !== null && (
                    <div className="mt-4 pt-4 border-t border-white/5 space-y-3">
                      <div className="text-[10px] text-white/60 leading-relaxed font-semibold bg-white/5 p-3 rounded-xl border border-white/5">
                        <span className="font-black text-blue-400 block mb-0.5 uppercase tracking-wider text-[8px]">AI Coach Explanation</span>
                      ...
                      <div className="flex justify-end">
                        ...
                        >
                          {currentQuestion + 1 < quizQuestions.length ? "Next Question" : "Complete Quiz"}
                        </button>
                      </div>
                    </div>
                  )}
        ...
        <div className="rounded-2xl border border-white/10 bg-black/40 p-6 backdrop-blur-xl space-y-5">
          <h2 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-1.5 border-b border-white/5 pb-4">
            <Sparkles className="h-5 w-5 text-amber-400 animate-spin" /> AI Study Recommendations
          </h2>

          <div className="space-y-4 max-h-[420px] overflow-y-auto pr-1">
            {loadingRecs ? (
              <div className="text-white/30 text-xs py-8 text-center flex items-center justify-center gap-1.5">
                <RefreshCw className="h-4 w-4 animate-spin text-amber-400" /> Computing pathways...
              </div>
            ) : recs.length === 0 ? (
              <div className="text-white/30 text-xs py-8 text-center">No recommendations loaded.</div>
            ) : (
              recs.map((rec, idx) => (
                <motion.div
                ...
                >
                  <div className="flex justify-between items-start">
                    <h3 className="text-xs font-black text-white leading-snug line-clamp-1">{rec.title}</h3>
                  ...
                  <p className="text-[10px] text-white/50 line-clamp-2">{rec.description}</p>
