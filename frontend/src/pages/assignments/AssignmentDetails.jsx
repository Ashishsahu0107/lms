...
export default function AssignmentDetails() {
      ...
      setUploadProgress((prev) => {
        if (prev >= 100) {
          ...
          const mockUrls = files.map(
            (f) => `/mock-uploads/${Date.now()}-${f.name}`,
          );
  ...
  const handleSubmitWork = async (e) => {
    ...
    if (!textAnswer.trim() && !uploadedFiles.length) {
      toast.error(
        "Please provide either a text answer or upload project attachments",
      );
      return;
    }
  ...
  if (loading) {
    return (
      <div
        ...
        id="assignment-details-loading"
      >
        <div className="loading loading-spinner loading-lg text-primary"></div>
        <p className="text-sm text-muted-foreground animate-pulse">
          Loading assignment sheet details...
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="text-center py-20 bg-base-100 rounded-3xl border border-base-300 shadow-xl max-w-lg mx-auto mt-10">
        ...
        <p className="text-muted-foreground text-sm mb-6">
          The requested assignment could not be retrieved. It may have been
          archived or deleted.
        </p>
    ...
    <div className="space-y-8" id="assignment-details-workspace">
      ...
      <div className="flex items-center justify-between">
        <Button
          onClick={() =>
            navigate(
              isStudent ? "/student/assignments" : "/teacher/assignments",
            )
          }
          variant="ghost"
        ...
        <span
          className={`badge ${isClosed ? "badge-secondary" : "badge-primary"} gap-1.5 py-3 px-3.5 rounded-2xl text-xs font-bold`}
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card
            className="bg-base-100 shadow-2xl border border-base-300 rounded-3xl overflow-hidden"
          ...
          >
            <div className="bg-gradient-to-r from-primary/10 via-base-100 to-base-100 p-8 border-b border-base-300">
              ...
              <div className="flex flex-wrap items-center gap-6 text-sm text-muted-foreground font-medium">
                ...
                <span className="flex items-center gap-2">
                  <Bookmark className="h-4.5 w-4.5 text-secondary" /> Type:{" "}
            ...
            <CardContent className="p-8 space-y-6">
              ...
              {assignment.description && (
                <div className="space-y-2">
                  <h3 className="text-lg font-bold text-foreground flex items-center gap-2">
                    <Sparkles className="h-5 w-5 text-primary" /> Overview
                    Description
                  </h3>
              ...
              {assignment.instructions && (
                <div className="space-y-3">
                  <h3 className="text-lg font-bold text-foreground flex items-center gap-2">
                    <FileText className="h-5 w-5 text-secondary" /> Assignment
              ...
              {assignment.attachments && assignment.attachments.length > 0 && (
                <div className="space-y-3">
                  <h3 className="text-md font-bold text-foreground">
                    Lecture Slides & Reference Documents
                  </h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {assignment.attachments.map((url, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className="flex items-center justify-between bg-base-200 p-3 rounded-xl border border-base-300 text-xs"
                        >
          ...
          {!isStudent && (
            <Card
              className="bg-base-100 shadow-2xl border border-base-300 rounded-3xl overflow-hidden"
              id="teacher-submissions-list-card"
            >
              <div className="p-6 bg-base-200/50 border-b border-base-300 flex items-center justify-between flex-wrap gap-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Grade and deliver constructive feedback reviews.
                  </p>
                </div>
                    ...
                    <tbody>
                      {submissionsList.length === 0 ? (
                        <tr>
                          ...
                          >
                            No student has submitted work for this assignment
                            yet.
                          </td>
                        </tr>
                      ) : (
                        submissionsList.map((sub) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td>
                                <span
                                  className={`badge rounded-lg px-2 py-1 text-xs font-bold text-white capitalize ${
                                    sub.status === "graded"
                                      ? "bg-success"
                                      : sub.status === "late"
                                        ? "bg-error"
                              ...
                              <td className="text-right">
                                <Button
                                  onClick={() =>
                                    navigate(
                                      `/${routePrefix}/assignments/submissions/${sub._id}`,
                                    )
                                  }
        ...
        {isStudent && (
          <div className="space-y-6">
            ...
            {submission && submission.status === "graded" && (
              <Card
                className="bg-gradient-to-br from-success/15 via-base-100 to-base-100 border border-success/30 shadow-2xl rounded-3xl overflow-hidden"
                id="student-graded-panel"
              >
            ...
            <Card
              className="bg-base-100 shadow-2xl border border-base-300 rounded-3xl overflow-hidden"
              id="student-submission-submit-panel"
            >
              <div className="p-6 bg-base-200/50 border-b border-base-300">
                <h3 className="font-bold text-lg">Your Submission Panel</h3>
                <p className="text-xs text-muted-foreground">
                  {submission
                    ? "Update and override your uploaded workspace below."
                    : "Deliver your response brief to get graded."}
                </p>
              </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="border-2 border-dashed border-base-300 bg-base-200 hover:bg-base-200/40 rounded-2xl p-6 text-center cursor-pointer transition-colors relative">
                      ...
                      <p className="text-[10px] text-muted-foreground mt-1">
                        Accepts PDF, ZIP, DOCX, images up to 25MB
                      </p>
                    </div>
                        ...
                        <div className="space-y-1.5 max-h-[150px] overflow-y-auto">
                          {uploadedFiles.map((fileUrl, idx) => {
                            ...
                            return (
                              <div
                                key={idx}
                                className="flex items-center justify-between bg-primary/5 border border-primary/20 p-2.5 rounded-xl text-xs"
                              >
                                <span className="font-mono text-primary font-semibold truncate max-w-[170px]">
                                  {name}
                                </span>
                                <button
                                  type="button"
