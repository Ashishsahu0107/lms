...
export default function TeacherDetails({ data = {}, onBack, onSendEmail }) {
    ...
    <div className="space-y-6" id="teacher-details-root">
      ...
      <div className="flex items-center justify-between">
        ...
        <Badge
          ...
          className="capitalize font-semibold text-xs border-0 px-3 py-1"
        >
        ...
        <Card className="lg:col-span-1 p-6 flex flex-col items-center justify-between space-y-6 bg-gradient-to-b from-card to-muted/20 border hover:shadow-md transition-all">
          ...
          <div className="w-full pt-4 border-t border-border">
            <Button
              ...
              onClick={() => onSendEmail(teacher.email)}
            >
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="p-6 hover:shadow-md transition-all">
            <h3 className="font-bold text-lg mb-4 flex items-center gap-2">
              <Briefcase className="h-5 w-5 text-blue-600" /> Academic &
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                <div className="bg-muted/30 border p-3.5 rounded-xl space-y-1">
                  ...
                  <span className="text-sm font-bold text-foreground capitalize">
                    {teacher.qualification || "Ph.D. Computer Science"}
                  </span>
                </div>
                <div className="bg-muted/30 border p-3.5 rounded-xl space-y-1">
                  <span className="text-xs text-muted-foreground font-semibold uppercase tracking-wider block">
                  ...
                  <span className="text-sm font-bold text-foreground capitalize">
                    {teacher.specialization || "Advanced React & AI"}
                  </span>
                </div>
                <div className="bg-muted/30 border p-3.5 rounded-xl space-y-1">
                  <span className="text-xs text-muted-foreground font-semibold uppercase tracking-wider block">
          ...
          <Card className="hover:shadow-md transition-all">
            <CardHeader className="pb-2 border-b">
              <CardTitle className="text-base font-semibold flex items-center gap-2">
                <BookOpen className="h-5 w-5 text-blue-600" /> Syllabus Assigned
                Courses
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {courses.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground text-sm">
                  This educator is not currently assigned to instruct active
                  courses.
                </div>
              ) : (
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody>
                      {courses.map((course) => (
                        ...
                        >
                          <td className="py-3 px-4 font-bold text-foreground truncate max-w-xs">
                            {course.title}
                          </td>
