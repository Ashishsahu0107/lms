  ...
  if (loading && view === "list") {
    return (
      <div
        ...
        id="admin-students-loading"
      >
        <Loader2 className="h-12 w-12 animate-spin text-blue-600" />
        <p className="text-muted-foreground text-sm font-medium animate-pulse">
          Syncing student archives...
        </p>
      </div>
    );
  }
            ...
            {confirmActionType === "delete" ? (
              <div className="flex gap-3 p-4 rounded-xl bg-red-500/10 text-red-600 border border-red-500/20">
                ...
                <p>
                  ...
                  <strong>{targetStudent.name}</strong>? This action terminates
                  all enrollments references keys and cannot be undone.
                </p>
              </div>
            ) : (
              <div className="flex gap-3 p-4 rounded-xl bg-amber-500/10 text-amber-600 border border-amber-500/20">
                <AlertCircle className="h-6 w-6 shrink-0" />
                <p>
                  Are you sure you want to {confirmActionType} student account{" "}
                  <strong>{targetStudent.name}</strong>? Suspended students
                  cannot log in or study course syllabus.
                </p>
              </div>
            )}
