...
export default function GeneralSettings({ settings, onSave }) {
  const [platformName, setPlatformName] = useState(
    settings?.platformName || "LMS Pro",
  );
  const [commissionRate, setCommissionRate] = useState(
    settings?.commissionRate || 20,
  );
      ...
      <Card className="hover:shadow-md border border-border bg-card">
        <CardHeader>
          <CardTitle className="text-base text-foreground font-bold flex items-center gap-2">
            <Building className="h-4 w-4 text-blue-500" /> Platform
            Configuration
          </CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground mt-1">
                Platform fee deducted from all student course enrollments.
              </p>
            </div>
            ...
            <div className="flex items-center justify-between p-3.5 rounded-xl bg-muted/30 border border-border">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">
                  Locks down student access to only view a landing screen.
                </p>
              </div>
              <button
                type="button"
      ...
      <Card className="hover:shadow-md border border-border bg-card">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground leading-relaxed">
            All general parameters configured here are mapped globally across
            billing controllers, user authentication routes, and lesson access
            gates.
          </p>
          <div className="space-y-2 pt-2 text-xs">
            ...
            <div className="flex items-center justify-between py-2 border-b border-border">
              ...
              <span className="font-bold text-emerald-600">
                {100 - (settings?.commissionRate ?? 20)}% Instructor /{" "}
                {settings?.commissionRate ?? 20}% App
              </span>
            </div>
