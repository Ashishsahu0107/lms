...
export default function AdminCourses() {
  ...
  async function loadAllData() {
    try {
      ...
      const teacherList =
        teachersRes.data?.data?.teachers || teachersRes.data?.teachers || [];
    ...
    >
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground">
            Moderate courses, assign teachers, and grant student enrollment
            access.
          </p>
        </div>
      </motion.div>
        ...
        {[
          ...
          <Card key={i} className="border border-base-300">
            <CardContent className="flex items-center gap-4 p-4">
              <div className={`p-3 rounded-md ${stat.bg} ${stat.text}`}>
      ...
      <motion.div variants={item} className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input
            className="pl-9 border-base-300"
            placeholder="Search courses..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
        </div>
        <Button variant="outline" className="gap-2 border-base-300">
      ...
      {loading ? (
        ...
        <Card className="p-12 text-center border bg-base-100/50">
          ...
          <p className="text-muted-foreground">
            There are no courses on the platform matching this selection.
          </p>
        </Card>
      ) : (
        <motion.div variants={item}>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b bg-base-200/50">
                    <th className="px-4 py-3.5 text-sm font-semibold text-muted-foreground">
                      Course Details
                    </th>
                    <th className="px-4 py-3.5 text-sm font-semibold text-muted-foreground">
                      Category
                    ...
                    <th className="px-4 py-3.5 text-sm font-semibold text-muted-foreground text-center">
                      Manage Access
                    </th>
                    <th className="px-4 py-3.5 text-sm font-semibold text-muted-foreground text-right">
                      Actions
                    </th>
                  </tr>
                </thead>
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2">
                          <Avatar
                            className="w-6 h-6"
                            fallback={course.teacherId?.name?.charAt(0) || "T"}
                          />
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex justify-center gap-2">
                          <Button
                            size="xs"
                            variant="outline"
                            className="gap-1 border-base-300"
                            onClick={() => {
                        ...
                        <div className="flex justify-end gap-1.5">
                          {course.status === "draft" ? (
                            ...
                            <Button
                              ...
                              onClick={() =>
                                handleUpdateStatus(course._id, "draft")
                              }
                            >
      ...
      <Modal
        ...
        title="Assign Course Instructor"
      >
        <form onSubmit={handleAssignTeacher} className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Select an instructor for course:{" "}
          ...
          <div>
            ...
            <select
              className="select select-bordered w-full h-10 px-3 bg-card border-base-300 rounded-md text-sm"
      ...
      <Modal
        ...
        title="Grant Student Enrollment Access"
      >
        <form onSubmit={handleEnrollStudent} className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Grant student access to course:{" "}
            <strong>{selectedCourse?.title}</strong>
          </p>
          <Input
      ...
      >
        {selectedCourse && (
          <div className="space-y-4">
            <div className="flex gap-3 p-4 rounded-md bg-error/10 border border-error/20">
              <AlertTriangle className="h-6 w-6 text-error shrink-0" />
              <div>
                <p className="text-sm font-bold text-error">
                  Warning: Critical Cascade Deletion
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Deleting "<strong>{selectedCourse.title}</strong>" will
                  permanently erase all nested Modules, Topic lectures, and
                  Student enrollment records from the database.
                </p>
              </div>
            </div>
