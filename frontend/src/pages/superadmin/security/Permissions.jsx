  ...
  return (
    <div className="space-y-6" id="permissions-root">
      <div className="flex items-center justify-between">
        ...
        <h2 className="text-lg font-bold text-foreground">Role Permissions Configuration</h2>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {roles.map((r, idx) => (
          <Card key={idx} className="hover:shadow-md border border-border flex flex-col justify-between bg-card">
            <div>
              ...
              <CardContent className="pt-5">
                <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">Authorized Capabilities:</h4>
                <ul className="space-y-3">
                  {r.caps.map((cap, i) => (
                    <li key={i} className="flex items-start gap-2 text-xs leading-relaxed text-muted-foreground">
                      <Check className="h-4 w-4 text-emerald-500 shrink-0 mt-0.5" /> <span>{cap}</span>
                    </li>
                  ))}
                </ul>
              </CardContent>
            ...
            <div className="p-4 border-t border-border mt-6 flex justify-end">
              <Button variant="outline" size="sm" className="gap-1.5 text-xs font-semibold" disabled>
                <Lock className="h-3.5 w-3.5 text-muted-foreground" /> Read-Only Role
              </Button>
            </div>
          </Card>
        ))}
