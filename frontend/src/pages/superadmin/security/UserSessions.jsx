import { ChevronLeft, Monitor, Trash2 } from "lucide-react";
import {
  Card,
  CardContent,
  CardHeader,
        ...
        <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-border">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Live logged-in devices across all roles.
            </p>
          </div>
          {sessions.length > 1 && (
            <Button
          ...
          {sessions.length === 0 ? (
            ...
            <div className="divide-y divide-border">
              {sessions.map((session, idx) => (
                <div
                  key={session._id || idx}
                  className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-5 gap-4"
                >
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="font-bold text-sm text-foreground">
                          {session.name || "Active Session User"}
                        </p>
                        <Badge
