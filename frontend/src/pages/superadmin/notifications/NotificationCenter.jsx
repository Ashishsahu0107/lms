...
import { Bell, AlertTriangle, CheckCircle2, Check, Trash2, CheckCircle } from "lucide-react";
    ...
    <div className="space-y-6" id="notification-center-root">
      <div className="flex justify-between items-center bg-card p-4 rounded-xl border border-border">
        <p className="text-sm font-semibold text-foreground">
          {unreadCount === 0 ? "You're all caught up! No unread notifications." : `${unreadCount} unread system notifications`}
        </p>
        {unreadCount > 0 && (
          <Button variant="outline" size="sm" className="gap-2 text-xs font-semibold" onClick={onMarkAllRead}>
      ...
      <div className="space-y-4">
        {notifications.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground bg-muted/20 rounded-xl border border-dashed">
            ...
            <p className="text-xs">Any system alerts or announcements will appear here.</p>
          </div>
        ) : (
          notifications.map((n) => (
            <Card key={n._id} className={`hover:shadow-sm transition-all border ${n.read ? "opacity-60 bg-card/60" : "border-blue-500/20 bg-card"}`}>
              <CardContent className="flex items-start gap-4 p-5">
                <div className={`p-2.5 rounded-xl ${
                  n.type === "warning" ? "bg-amber-500/10 text-amber-500" :
                  n.type === "danger" || n.type === "critical" ? "bg-red-500/10 text-red-500" :
