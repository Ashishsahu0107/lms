...
export const studentService = {
  ...
  async updateProgress(studentId, courseId, topicId) {
    ...
    const idx = progress.lectureProgress.findIndex(
  ...
  async submitAssignment(studentId, assignmentId, { fileUrl, notes }) {
    ...
    await awardXPAndCheckStreak(studentId, "SUBMIT_ASSIGNMENT", {
      assignmentId,
    });
  ...
  async submitQuiz(studentId, quizId, { answers, score }) {
    ...
    const accuracy =
      quiz.totalMarks > 0 ? Math.round((score / quiz.totalMarks) * 100) : 100;
    ...
    let attempt = await QuizAttempt.create({
      ...
      answers: answers.map((ans) => ({
        ...
        selectedAnswers: Array.isArray(ans.selectedAnswers)
          ? ans.selectedAnswers
          : [ans.selectedAnswers],
      })),
      score,
    ...
    await awardXPAndCheckStreak(studentId, "COMPLETE_QUIZ", {
      quizId,
      accuracy,
    });
  ...
  async getCertificate(studentId, courseId) {
    ...
    if (!cert)
      throw new NotFoundError(
        "Certificate has not been generated or approved yet.",
      );
    return cert;
  },
  ...
  async getProgressDetails(studentId, courseId) {
    ...
    const modulesWithCompletion = (course.modules || []).map((mod) => {
      ...
      const completedInMod = topics.filter((t) =>
        completedTopicIds.has(t._id.toString()),
      ).length;
      const modCompletion =
        totalInMod > 0 ? Math.round((completedInMod / totalInMod) * 100) : 0;
  ...
  async getStudentAnalyticsInsights(studentId) {
    const user = await User.findById(studentId).select(
      "xp streak badges achievements enrolledCourses",
    );
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      weeklyStudy.push({
        month: days[dayIndex],
        hours: parseFloat(
          (baseHours + (completedThatDay ? 1.2 : 0)).toFixed(1),
        ),
      });
    }
    ...
    if (user.streak >= 3) {
      insights.push(
        `🔥 Incredible job! You have kept your active learning streak alive for ${user.streak} days. Keep it up!`,
      );
    } else {
      insights.push(
        `💡 Tip: Check in daily and complete at least one topic to start an active study streak and unlock bonuses.`,
      );
    }

    if (avgAccuracy >= 85) {
      insights.push(
        `🎯 Your quiz accuracy is a stellar ${avgAccuracy}%. You exhibit strong topic comprehension!`,
      );
    } else if (avgAccuracy > 0) {
      insights.push(
        `📚 Your average quiz score is ${avgAccuracy}%. Review lecture summaries before quiz attempts to boost scores.`,
      );
    }

    if (completedCoursesCount > 0) {
      insights.push(
        `🎓 Congratulations! You have fully completed ${completedCoursesCount} course certificate maps.`,
      );
    } else if (progressList.length > 0) {
      const close = progressList.find(
        (p) => p.progress >= 70 && p.progress < 100,
      );
      if (close) {
        insights.push(
          `🚀 Almost there! You are ${100 - close.progress}% away from completing '${close.courseId?.title || "your course"}' and generating a secure certificate.`,
        );
      }
    }
