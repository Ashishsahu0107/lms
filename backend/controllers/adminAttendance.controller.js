...
export async function getAdminAttendanceAnalyticsController(req, res, next) {
  try {
    ...
    const [totalRecords, recentRecords, totalCourses, totalStudents] = await Promise.all([
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      dailyMap[key] = { date: key, present: 0, absent: 0, late: 0, leave: 0, total: 0 };
    }
    ...
    recentRecords.forEach((r) => {
      ...
      if (!studentAttendance[key]) {
        studentAttendance[key] = { studentId: r.studentId, courseId: r.courseId, total: 0, present: 0 };
      }
      studentAttendance[key].total++;
      if (r.status === "present" || r.status === "late") studentAttendance[key].present++;
    });
...
export async function getAdminAttendanceReportsController(req, res, next) {
  try {
    const { courseId, teacherId, from, to, status, page = 1, limit = 50 } = req.query;
